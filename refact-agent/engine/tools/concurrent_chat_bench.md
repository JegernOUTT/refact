...
The five performance switches are enabled by default: `trajectory_writer_enabled`, `trajectory_index_coordinator_enabled`, `trajectory_watcher_self_write_enabled`, `tool_catalog_snapshots_enabled`, and `vecdb_path_coalescing_enabled`. Configure them through the complete `POST /v1/trajectory-settings` configuration returned by `GET /v1/trajectory-settings`; do not construct a partial payload because omitted fields reset to defaults. `REFACT_TRAJECTORY_WRITER`, `REFACT_TRAJECTORY_INDEX_COORDINATOR`, `REFACT_TRAJECTORY_WATCHER_SELF_WRITE`, `REFACT_TOOL_CATALOG_SNAPSHOTS`, and `REFACT_VECDB_PATH_COALESCING` are recovery overrides that take precedence over those persisted settings and accept truthy `1`, `true`, `yes`, or `on`; use `0` or `false` to disable one switch and recover its legacy path. Persisted writer, index coordinator, tool catalog, and VecDB setting changes require an engine restart; watcher self-write suppression changes apply live. Trajectory JSON and index schema stay unchanged, and disabled VecDB coalescing returns to FIFO regular-path enqueueing without migration or data deletion.
