...
The five temporary switches are default-off until final acceptance: `REFACT_TRAJECTORY_WRITER`, `REFACT_TRAJECTORY_INDEX_COORDINATOR`, `REFACT_TRAJECTORY_WATCHER_SELF_WRITE`, `REFACT_TOOL_CATALOG_SNAPSHOTS`, and `REFACT_VECDB_PATH_COALESCING`. Set only the intended switch to a truthy value (`1`, `true`, `yes`, or `on`) and restart the affected engine process. Recovery is immediate: set that switch to `0` or unset it, then restart; trajectory JSON and index schema stay unchanged, and VecDB returns to FIFO regular-path enqueueing without migration or data deletion.
