...
pub async fn forward_to_openai_style_endpoint(
    ...
    if !sampling_parameters.stop.is_empty() {
        // openai does not like empty stop
    ...
    if model_rec.support_metadata {
        headers.insert(
            ...
            HeaderValue::from_str(
                format!("refact-lsp {}", crate::version::build::PKG_VERSION).as_str(),
            )
            .unwrap(),
        );
    }
...
fn passthrough_messages_to_json(data: &mut serde_json::Value, prompt: &str, model_name: &str) {
...
pub async fn get_embedding_openai_style(
    ...
    if model_rec.base.api_key.is_empty() {
        return Err(format!(
            "Cannot access embedding model, because api_key is empty"
        ));
    }
    ...
    let json = response.json::<serde_json::Value>().await.map_err(|err| {
        format!(
            "get_embedding_openai_style: failed to parse the response: {:?}",
            err
        )
    })?;
    ...
    match serde_json::from_value::<Vec<EmbeddingsResultOpenAI>>(json["data"].clone()) {
        Ok(unordered) => {
            for ures in unordered.into_iter() {
                ...
                if index >= B {
                    return Err(format!(
                        "get_embedding_openai_style: index out of bounds: {:?}",
                        json
                    ));
                }
