        ...
        if self.remote_docker {
            Some(SshConfig {
                ...
                identity_file: if !self.ssh_identity_file.is_empty() {
                    Some(self.ssh_identity_file.clone())
                } else {
                    None
                },
            })
...
impl ToolDocker {
    pub async fn command_execute(
        ...
        fail_if_stderr_is_not_empty: bool,
        verbose_error: bool,
        ...
        if command_is_interactive_or_blocking(&command_args) {
            return Err(
                "Docker commands that are interactive or blocking are not supported".to_string(),
            );
        }
    ...
    pub async fn get_docker_host(
        ...
        match &self.settings_docker.get_ssh_config() {
            Some(ssh_config) => {
                let local_port = forward_remote_docker_if_needed(
                    &self.settings_docker.docker_daemon_address,
                    ssh_config,
                    gcx.clone(),
                )
                .await?;
...
impl Tool for ToolDocker {
    ...
    async fn tool_execute(
        ...
        let (stdout, _) = self
            .command_execute(&command, gcx.clone(), true, false)
            .await?;
...
fn command_is_interactive_or_blocking(command_args: &Vec<String>) -> bool {
    ...
    let subcommand_generic = command_args_iter
        .next()
        .map(|arg| arg.as_str())
        .unwrap_or("");
