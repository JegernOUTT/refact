...
impl Drop for PdbSession {
    fn drop(&mut self) {
        self.process
            .start_kill()
            .map_err(|e| error!("Failed to kill process: {}", e))
            .ok();
    }
}
...
impl Tool for ToolPdb {
    async fn tool_execute(
        ...
        if command_args.iter().any(|x| x.starts_with("python")) {
            if !command_args.windows(2).any(|w| w == ["-m", "pdb"]) {
                let python_index = command_args
                    .iter()
                    .position(|x| x.starts_with("python"))
                    .ok_or("Open a new session by running pdb(\"python -m pdb my_script.py\")")?;
                command_args.insert(python_index + 1, "pdb".to_string());
                command_args.insert(python_index + 1, "-m".to_string());
            }
            let output = start_pdb_session(
                &python_command,
                &mut command_args,
                &session_hashmap_key,
                &workdir_maybe,
        ...
        let output_result = match command_args[0].as_str() {
            "wait" => {
                ...
                let timeout_seconds = command_args[1].parse::<u64>().map_err(|_| {
                    "Argument `n_seconds` in `wait n_seconds` is not a number".to_string()
                })?;
                interact_with_pdb(
                    "",
                    &mut pdb_session,
    ...
    fn tool_description(&self) -> ToolDesc {
        ToolDesc {
            name: "pdb".to_string(),
            display_name: "Python Debugger".to_string(),
            source: ToolSource {
...
async fn start_pdb_session(
    ...
    info!(
        "Starting pdb session with command: {} {:?}",
        python_command, command_args
    );
    ...
    let stdin = process
        .stdin
        .take()
        .ok_or("Failed to open stdin for pdb process")?;
    let stdout = BufReader::new(
        process
            .stdout
            .take()
            .ok_or("Failed to open stdout for pdb process")?,
    );
    let stderr = BufReader::new(
        process
            .stderr
            .take()
            .ok_or("Failed to open stderr for pdb process")?,
    );
    let mut pdb_session = PdbSession {
        process,
        stdin,
        stdout,
        stderr,
...
async fn interact_with_pdb(
    ...
    if !input_command.is_empty() {
        let (prev_output, prev_error, _) = blocking_read_until_token_or_timeout(
            &mut pdb_session.stdout,
            &mut pdb_session.stderr,
            100,
            PDB_TOKEN,
        )
        .await?;
...
fn format_all_output(
    output_main_command: &str,
    error_main_command: &str,
    output_list: &str,
    error_list: &str,
    output_where: &str,
    error_where: &str,
