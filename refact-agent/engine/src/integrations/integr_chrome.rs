...
async fn setup_chrome_session(
    ...
    let window_size = match (
        args.window_width.parse::<u32>(),
        args.window_height.parse::<u32>(),
    ) {
...
fn get_inner_html(element: &Element) -> Result<String, String> {
    let func = r"
    function() {
    ...
    let result = element
        .call_js_fn(func, vec![], false)
        .map_err(|e| e.to_string())?;
    Ok(result.value.unwrap().to_string())
}

fn format_remote_object(remote_object: &RemoteObject) -> String {
    let mut result = vec![];
...
async fn session_open_tab(
    ...
    match chrome_session.tabs.get(tab_id) {
        ...
        None => {
            let headless_tab = chrome_session
                .browser
                .new_tab()
                .map_err(|e| e.to_string())?;
...
async fn chrome_command_exec(
    ...
    match cmd {
        Command::OpenTab(args) => {
            let log = {
                ...
                let chrome_session = chrome_session_locked
                    ...
                    .downcast_mut::<ChromeSession>()
                    .ok_or("Failed to downcast to ChromeSession")?;
                session_open_tab(chrome_session, &args.tab_id, &args.device, &settings_chrome)
                    .await?
            };
            tool_log.push(log);
        }
                    ...
                    if elements.len() == 0 {
                        ...
                        if elements.len() > 2 {
                            elements_log.push(format!(
                                "\n\nShown html for first of {} elements",
                                elements.len()
                            ));
                        }
        ...
        Command::PressKey(args) => {
            ...
            let log = {
                ...
                match {
                    tab_lock
                        .headless_tab
                        .press_key_with_modifiers(args.key.as_str(), args.key_modifiers.as_deref())
                        .map_err(|e| e.to_string())?;
                    tab_lock
                        .headless_tab
                        .wait_until_navigated()
        ...
        Command::TabLog(args) => {
            ...
            let filtered_log =
                output_mini_postprocessing(&OutputFilter::default(), tab_log.as_str());
        ...
        Command::Eval(args) => {
            ...
            let log = {
                ...
                match tab_lock
                    .headless_tab
                    .evaluate(args.expression.as_str(), false)
                {
                    Ok(remote_object) => format_remote_object(&remote_object),
                    Err(e) => {
                        format!(
                            "eval failed at {}: {}",
                            tab_lock.state_string(),
        ...
        Command::Styles(args) => {
            ...
            let log = {
                ...
                match {
                    tab_lock
                        .headless_tab
                        .call_method(DOMEnable {
                            include_whitespace: None,
                        })
                        .map_err(|e| e.to_string())?;
                    tab_lock
                        .headless_tab
                        .call_method(CSSEnable(None))
                        .map_err(|e| e.to_string())?;
                    let element = tab_lock
                        .headless_tab
                        .find_element(&args.selector)
                        .map_err(|e| e.to_string())?;
                    let computed_styles =
                        element.get_computed_styles().map_err(|e| e.to_string())?;
                    let mut styles_filtered = computed_styles
                        .iter()
                    ...
                    if styles_filtered.len() > max_lines_output {
                        let skipped_message = format!(
                            "Skipped {} properties. Specify filter if you need to see more.",
                            styles_filtered.len() - max_lines_output
                        );
                    ...
                    Ok(styles_str) => {
                        format!(
                            "Style properties for element `{}` at {}:\n{}",
                            args.selector,
                            tab_lock.state_string(),
                            styles_str
                        )
...
fn parse_single_command(command: &String) -> Result<Command, String> {
    ...
    match command_name.as_str() {
        "open_tab" => match parsed_args.as_slice() {
            [tab_id, device_str] => {
                let device = match device_str.as_str() {
                    ...
                    _ => {
                        return Err(format!(
                            "unknown device type: {}. Should be `desktop`, `mobile` or `tablet`.",
                            parsed_args[0]
                        ))
                    }
                };
            ...
            _ => Err(
                "Missing one or several arguments `tab_id`, `<device|mobile|tablet>`".to_string(),
            ),
        },
        "navigate_to" => match parsed_args.as_slice() {
            [tab_id, uri] => Ok(Command::NavigateTo(NavigateToArgs {
                uri: uri.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `uri`".to_string()),
        },
        "scroll_to" => match parsed_args.as_slice() {
            [tab_id, selector] => Ok(Command::ScrollTo(TabElementArgs {
                selector: selector.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `selector`".to_string()),
        },
        "screenshot" => match parsed_args.as_slice() {
            [tab_id] => Ok(Command::Screenshot(TabArgs {
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`".to_string()),
        },
        "html" => match parsed_args.as_slice() {
            [tab_id, selector] => Ok(Command::Html(TabElementArgs {
                selector: selector.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `selector`".to_string()),
        },
        "reload" => match parsed_args.as_slice() {
            [tab_id] => Ok(Command::Reload(TabArgs {
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`".to_string()),
        },
        "click_at_point" => match parsed_args.as_slice() {
            [tab_id, x_str, y_str] => {
                let x = x_str
                    .parse::<f64>()
            ...
            _ => Err("Missing one or several arguments `tab_id`, `x`, 'y`".to_string()),
        },
        "click_at_element" => match parsed_args.as_slice() {
            [tab_id, selector] => Ok(Command::ClickAtElement(TabElementArgs {
                selector: selector.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `selector`".to_string()),
        },
        "type_text_at" => match parsed_args.as_slice() {
            [tab_id, text] => Ok(Command::TypeTextAt(TypeTextAtArgs {
                text: text.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `text`".to_string()),
        },
        "press_key" => match parsed_args.as_slice() {
            ...
            [tab_id, key, key_modifiers] => {
                let modifiers: Result<Vec<ModifierKey>, String> = key_modifiers
                    .split(',')
            ...
            _ => Err("Missing one or several arguments `tab_id`, `key`".to_string()),
        },
        "tab_log" => match parsed_args.as_slice() {
            [tab_id] => Ok(Command::TabLog(TabArgs {
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`".to_string()),
        },
        "eval" => match parsed_args.as_slice() {
            [tab_id, expression] => Ok(Command::Eval(EvalArgs {
                expression: expression.clone(),
                tab_id: tab_id.clone(),
            })),
            _ => Err("Missing one or several arguments `tab_id`, `expression`.".to_string()),
        },
        "styles" => match parsed_args.as_slice() {
            [tab_id, selector, property_filter] => Ok(Command::Styles(StylesArgs {
                selector: selector.clone(),
                tab_id: tab_id.clone(),
        ...
        "wait_for" => match parsed_args.as_slice() {
            [tab_id, seconds_str] => {
            ...
            _ => Err("Missing one or several arguments `tab_id`, `seconds`.".to_string()),
        },
