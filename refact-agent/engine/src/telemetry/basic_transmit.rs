...
pub async fn send_telemetry_data(
    ...
    let resp_maybe = http_client
        .post(telemetry_dest.clone())
        .body(contents)
        .header(
    ...
    let resp_json =
        serde_json::from_str::<serde_json::Value>(&resp_body).unwrap_or_else(|_| json!({}));
...
pub async fn send_telemetry_files_to_mothership(
    ...
    for path in files {
                ...
                .any(|s| path_str.ends_with(s))
        {
            ...
            let resp =
                send_telemetry_data(contents, &telemetry_basic_dest, &api_key, gcx.clone()).await;
            if resp.is_err() {
        ...
        if res.is_err() {
            error!(
                "telemetry send success, but cannot move file: {}",
                res.err().unwrap()
            );
...
pub async fn basic_telemetry_compress(global_context: Arc<ARwLock<GlobalContext>>) {
