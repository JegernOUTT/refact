...
fn snippet_register(ss: &SaveSnippet, grey_text: String, context_used: bool) -> u64 {
...
pub fn snippet_register_from_data4cache(
    ...
    data4cache.completion0_snippet_telemetry_id = Some(snippet_register(
        &ss,
        data4cache.completion0_text.clone(),
        context_used,
    ));
}
...
pub async fn sources_changed(
    ...
    basic_robot_human::create_robot_human_record_if_not_exists(
        &mut storage_locked.tele_robot_human,
        uri,
        text,
    );
    ...
    for snip in storage_locked.tele_snippets.iter_mut() {
        ...
        let cursor_file_path = canonical_path(&snip.inputs.cursor.file)
            .to_string_lossy()
            .to_string();
        ...
        if !accepted_snippets
            .iter()
            .any(|s: &SnippetTracker| s.snippet_telemetry_id == snip.snippet_telemetry_id)
        {
            accepted_snippets.push(snip.clone());
            debug!(
                "sources_changed: ID{}: snippet is added to accepted",
                snip.snippet_telemetry_id
            );
        }

        let (grey_valid, mut grey_corrected) =
            utils::if_head_tail_equal_return_added_text(orig_text.unwrap(), text, &snip.grey_text);
        if grey_valid {
            let unchanged_percentage =
                utils::unchanged_percentage(&grey_corrected, &snip.grey_text);
            ...
            if snip.remaining_percentage >= 0. {
                ...
                debug!(
                    "ID{}: snippet is finished, remaining_percentage={}",
                    snip.snippet_telemetry_id, snip.remaining_percentage
                );
            } else {
    ...
    for snip in accepted_snippets {
        basic_robot_human::increase_counters_from_accepted_snippet(
            &mut storage_locked,
            uri,
            text,
            &snip,
        );
        basic_comp_counters::create_data_accumulator_for_accepted_snippet(
            &mut storage_locked.snippet_data_accumulators,
            uri,
            &snip,
        );
    }
    basic_robot_human::on_file_text_changed(&mut storage_locked.tele_robot_human, uri, text);
    basic_comp_counters::on_file_text_changed(
        &mut storage_locked.snippet_data_accumulators,
        uri,
        text,
    );
