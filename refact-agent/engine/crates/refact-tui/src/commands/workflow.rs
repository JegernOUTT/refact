...
pub const GOAL_COMMAND: CommandDef = CommandDef {
    name: "goal",
    aliases: &[],
    description: "structured prompt: clarify the implementation goal",
    args_hint: "",
    availability: CommandAvailability::IdleOnly,
    action: CommandAction::Workflow {
        command: WorkflowCommand::GoalPrompt,
    },
};
...
pub fn goal_prompt() -> &'static str {
    "Help me clarify the current goal. Restate the goal, list assumptions, propose acceptance criteria, identify blockers, and ask only the smallest set of questions needed before implementation."
}
