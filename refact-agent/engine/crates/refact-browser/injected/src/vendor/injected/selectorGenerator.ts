...
export type SelectorGeneratorRuntime = {
  _evaluator: SelectorEvaluatorImpl;
  parseSelector(selector: string): ParsedSelector;
  querySelector(selector: ParsedSelector, root: Document | Element, strict: boolean): Element | undefined;
  querySelectorAll(selector: ParsedSelector, root: Document | Element): Element[];
};

import type { ParsedSelector } from '../isomorphic/selectorParser';
import type { SelectorEvaluatorImpl } from './selectorEvaluator';

function splitTestIdAttributeNames(testIdAttributeName: string): string[] {
...
const kTextScore = 120;
const kLabelScore = 140;
const kPlaceholderScore = 160;
const kAltTextScore = 180;
...
export function generateSelector(injectedScript: SelectorGeneratorRuntime, targetElement: Element, options: GenerateSelectorOptions): { selector: string, elements: Element[] } {
...
function generateSelectorFor(cache: Cache, injectedScript: SelectorGeneratorRuntime, targetElement: Element, options: InternalOptions): SelectorToken[] | null {
...
function buildNoTextCandidates(injectedScript: SelectorGeneratorRuntime, element: Element, options: InternalOptions): SelectorToken[] {
...
function buildTextCandidates(injectedScript: SelectorGeneratorRuntime, element: Element, isTargetNode: boolean, options: InternalOptions): SelectorToken[][] {
...
function cssFallback(injectedScript: SelectorGeneratorRuntime, targetElement: Element, options: InternalOptions): SelectorToken[] {
