...
pub fn detect_routes(lang: &str, text: &str) -> Vec<DetectedRoute> {
            ...
            .map(|n| node_text(n, bytes).to_string())
        {
            ...
            for child in node.children(&mut cursor) {
                if child.kind() == "decorator" {
                    if let Some(route) = python_decorator_route(node_text(child, bytes), &handler) {
                        out.push(route);
                    }
                }
            }
...
fn python_decorator_route(text: &str, handler: &str) -> Option<DetectedRoute> {
    let call = text.trim().strip_prefix('@').unwrap_or(text).trim();
    let before = call.split('(').next()?.trim();
    let method = before.rsplit('.').next()?.to_ascii_lowercase();
    ...
    Some(DetectedRoute {
        ...
        path: first_string_arg_from_call(call).unwrap_or_default(),
...
"#;
        ...
        assert!(has(&routes, "post", "create_user"), "got {routes:?}");
        assert!(has(&routes, "ANY", "health"), "got {routes:?}");
    }
...
"#;
        let routes = detect_routes("typescript", src);
...
"#;
        let routes = detect_routes("go", src);
...
"#;
        let routes = detect_routes("rust", src);
        assert!(has(&routes, "GET", "list_users"), "got {routes:?}");
...
"#;
        let routes = detect_routes("java", src);
        assert!(has(&routes, "GET", "listUsers"), "got {routes:?}");
    }
...
"#;
        let routes = detect_routes("php", src);
    ...
    fn detects_rails_routes() {
        ...
        let routes = detect_routes("ruby", src);
        assert!(has(&routes, "get", "index"), "got {routes:?}");
    }
