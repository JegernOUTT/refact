...
impl LangExtractor for CppExtractor {
...
fn declarator_name(node: Node, bytes: &[u8]) -> Option<String> {
    match node.kind() {
        ...
        | "operator_name" => Some(node_text(node, bytes).to_string()),
        "qualified_identifier" => node
            .child_by_field_name("name")
            .and_then(|n| declarator_name(n, bytes))
            .or_else(|| Some(node_text(node, bytes).to_string())),
        "function_declarator"
        | "pointer_declarator"
...
fn base_classes(node: Node, bytes: &[u8]) -> Vec<String> {
    let mut out = Vec::new();
...
mod tests {
    ...
    fn extract(source: &str) -> (Vec<SymbolNode>, Vec<RawRef>) {
        let tree = CppExtractor::parse(source).expect("parse cpp");
        CppExtractor.extract(&tree, source)
    }

    #[test]
    fn extracts_free_functions_and_calls() {
        let src = "\
int helper() { return 1; }
...
";
        ...
        assert!(paths.contains(&"compute".to_string()), "got {paths:?}");
        assert!(
            refs.iter()
                .any(|r| r.name == "helper" && r.from == "compute"),
            "got {refs:?}"
        );
    }
