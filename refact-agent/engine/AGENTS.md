...
- There is no per-episode segment-summary attempt cap. Once triggered (proactively at High/Critical pressure or reactively on a context-limit error), `run_reserved_segment_summarization` loops, summarizing the largest budget-fitting eligible span each pass until the chat has been reduced by at least `COMPRESSION_TARGET_REDUCTION_PERCENT` (50%) of the episode-start tokens or no eligible/fitting span remains; oversize old history is compressed as several consecutive budget-sized segments. Candidates never include sources already covered by an active summary (covered messages split segments, preventing overlapping re-summarization of the same span), spans must contain summarizable text (image-only spans are ineligible), and the post-await apply revalidates that every pre-await source message is still present with an unchanged `source_hash` before inserting the summary (`SourceChanged` skips are not remembered as insufficient). It returns `CompactionOutcome::{Applied, NothingToCompact, LlmUnavailable}`. `thread.reactive_compact_attempts` no longer gates summarization — it only bounds the in-call context-limit retry rounds (`MAX_CONTEXT_LIMIT_COMPACTION_ROUNDS`), resets on any successful generation and on `reset_compaction_runtime_state` (manual `ctx_apply`, restores, external reloads), and is runtime-only: trajectory saves strip the key and loads ignore injected values, so a fresh session always starts at `None`.
...
**Categories**: Codebase search (CodeGraph definitions, tree, cat, regex, semantic memory) · CodeGraph analysis (overview, health, git risk, why, duplication, dead code, security scan, PR blast, map) · Codebase change (create/update/rm/mv/undo/apply_patch — confirmation required) · Web (fetch, search, Chrome automation) · Code execution (shell, process_*, sleep, cron_*) · System integrations (cmdline_*, service_*) · Knowledge (search, create, trajectories) · Agent (subagent, strategic_planning, deep_research, review — a multi-agent review swarm with quick/standard/deep depths; see `docs/review_pipeline.md`) · Task management (~18 tools) · IDE (open_file, paste_text) · Integration-defined + MCP tools.
