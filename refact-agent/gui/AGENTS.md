...
These nine tools return **JSON** shaped by `engine/src/codegraph/code_intel_api.rs` — the same response structs the `/v1/code-intel/*` HTTP endpoints serialize. Every result is `{ "tool": "<name>", "summary": "<one line>", ...payload }`. Never text-parse a tool result.
...
- `SettingItem` rows are FLUID (the settings width doctrine): `.item` declares no `max-width` at all — rows fill the parent section track so every control shares one right edge; readability comes from the copy column's absolute cap (68ch family) and the row-layout control cap (`--rf-setting-item-control-max`), never from a row-level measure. When a cap is genuinely needed anywhere in this primitive it must be an absolute length; a percentage `max-width` such as `100%` or `min(100%, …)` is forbidden. A percentage `max-width` on these nested grid boxes makes each row's intrinsic-size contribution cyclic, so Blink re-runs intrinsic layout per row: a 60-row settings list measured ~25ms of layout **per row** (≈1.7s per layout pass, main thread ~85% blocked, every interaction multi-second) versus ~0.6ms per row with absolute lengths, at pixel-identical geometry from 360px to 1400px. Containment inside the row comes from `min-width: 0` plus the parent grid track (`minmax(0, 1fr)` / `minmax(0, var(--rf-setting-item-control-max))`), not from percentage caps. `SettingItem.test.tsx` guards this with a CSS characterization test. Prefer absolute caps over percentage `max-width` in any primitive that renders in long repeated lists.
...
- **Performance**: The Toolbar menu opens the `performance` page, implemented in `features/Performance`. `PerformancePage` fetches `GET /v1/performance/telemetry` with visible-tab five-second polling and uses its POST enable/disable and reset endpoints for process-local aggregate collection. Its trajectory settings panel fetches `GET /v1/trajectory-settings` and posts the complete server-loaded configuration to `POST /v1/trajectory-settings`; preserve every returned key, including fields without a descriptor, because the endpoint treats omitted fields as defaults and a metadata-rebuilt payload can silently lose settings.
