      ...
      {(["light", "dark"] as const).map((appearance) => (
        <section className={styles.panel} data-appearance={appearance} key={appearance}>
          <div className={styles.header}>
            ...
            <p className={styles.description}>Hover or focus the trigger to show a clamped tooltip.</p>
          </div>
          <Tooltip>
            <Tooltip.Trigger asChild>
              <button className={styles.button} type="button">Hover or focus me</button>
            </Tooltip.Trigger>
            <Tooltip.Content>
              Tooltip content wraps and clamps to the viewport while preserving theme tokens.
            </Tooltip.Content>
          </Tooltip>
        </section>
      ))}
...
export const ReducedMotion: Story = { render: () => <TooltipStory reducedMotion /> };
