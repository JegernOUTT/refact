...
vi.mock("./IndexingSettingsSection", () => ({
  IndexingSettingsSection: () => <div data-testid="indexing-section" />,
}));
...
describe("SettingsHub — section routing by page name", () => {
  ...
  it("shows Indexing section for indexing settings page", () => {
    renderHub({ name: "indexing settings" });
    expect(screen.getByTestId("indexing-section")).toBeInTheDocument();
  });

  it("shows Privacy section for privacy settings page", () => {
    renderHub({ name: "privacy settings" });
...
describe("SettingsHub — left nav dispatches change(), not push()", () => {
  ...
  it("switches to privacy section via change (stack length unchanged)", () => {
    const { store } = renderHub({ name: "general settings" });
    const initialLength = store.getState().pages.length;
    ...
    const pages = store.getState().pages;
    expect(pages.length).toBe(initialLength);
    expect(pages[pages.length - 1].name).toBe("privacy settings");
  });

  it("switches to indexing section via change (stack length unchanged)", () => {
    const { store } = renderHub({ name: "general settings" });
    const initialLength = store.getState().pages.length;
