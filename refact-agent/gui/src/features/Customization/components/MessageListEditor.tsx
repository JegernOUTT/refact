...
import { Flex, Button, TextField, IconButton, TextArea, Text, DropdownMenu } from "@radix-ui/themes";
import { PlusIcon, TrashIcon, ChevronUpIcon, ChevronDownIcon, ChevronDownIcon as DropdownIcon } from "@radix-ui/react-icons";
...
export const MessageListEditor: React.FC<MessageListEditorProps> = ({
  ...
  const updateMessage = useCallback((id: string, field: keyof MessageTemplate, fieldValue: string) => {
    emit(internal.map((m) => m._id === id ? { ...m, [field]: fieldValue } : m));
  }, [internal, emit]);
  ...
  const moveMessage = useCallback((id: string, direction: -1 | 1) => {
    ...
    [newInternal[idx], newInternal[newIdx]] = [newInternal[newIdx], newInternal[idx]];
    emit(newInternal);
  }, [internal, emit]);
  ...
  return (
    <Flex direction="column" gap="2">
      ...
      {value.length === 0 && (
        <Text size="1" color="gray">No messages defined</Text>
      )}
      {internal.map((msg, index) => (
          <Flex key={msg._id} direction="column" gap="2" className={styles.messageItem}>
                ...
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Content>
                    {COMMON_ROLES.map((role) => (
                      <DropdownMenu.Item key={role} onSelect={() => updateMessage(msg._id, "role", role)}>
                        {role}
                      </DropdownMenu.Item>
                    ))}
                  </DropdownMenu.Content>
