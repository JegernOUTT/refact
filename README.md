...
- **Mode as behavior, not branding:** `ask`, `explore`, `plan`, `agent`, `debug`, `buddy`, `task_planner`, and `task_agent` ship as separate defaults with distinct prompts, toolsets, subagent policies, and execution discipline.
- **Read-only when you need bearings, sharp tools when you need motion:** Explore gathers context without editing; Plan turns decisions into implementation shape; Agent operates autonomously with file, shell, process, MCP, knowledge, and task tools.
