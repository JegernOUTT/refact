...
| N-56 | **S3** | open | **Confirmation actions-row gap splits 8px vs 12px across the same family** (supersedes and quantifies S3-11's two-point observation). Measured: **8px** — `DeletePopover` (104.2 - 96.2), CreateWorktree (prior ledger); **12px** — ModeTransitionDialog x2, TaskPlannerDialog x2, AddProviderInstance (prior ledger). Five-to-two split for an identical `Cancel + primary` row. Both values sit on the spacing scale, so no off-scale census can catch it — it needs one `--rf-actions-gap` decision. |
